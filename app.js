...
galMap.init = function() {
  ...
  for (var i in $scope.nodes){
    ...
    starImage.setAttribute('points',$scope.spikes(system.links.length));
...
app.directive('starLink',function(){
  return{
    ...
    restrict: 'E',
    replace:true,
    template: '<line class="link{{link.value}}" ng-hide="{{link.show}}" stroke-width=1 ng-attr-x1="{{node.x +600}}" ng-attr-y1="{{node.y +600}}" ng-attr-x2="{{nodes[link.destination].x +600}}" ng-attr-y2="{{nodes[link.destination].y +600}}"> </line>'
  };
});

app.directive('mapMenu', function(){
  return {
    restrict: 'E',
    replace: true,
